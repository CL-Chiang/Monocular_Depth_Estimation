...
class InferenceHelper:
    def __init__(self, dataset='nyu', device='cuda:0', n_bins=256, weight_path="./pretrained/AdaBins_nyu.pt"):
        ...
        if dataset == 'nyu':
            ...
            model = UnetAdaptiveBins.build(n_bins=n_bins, min_val=self.min_depth, max_val=self.max_depth)
...
if __name__ == '__main__':
    ...
    # Need To Specify #
    ###################
    img_path = "classroom_rgb_00283.jpg"
    weight_path = "checkpoints/Model5_3.pt"
    n_bins = 80

    img = Image.open(img_path).resize((640, 480))
    start = time()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    inferHelper = InferenceHelper(device=device, n_bins=n_bins, weight_path=weight_path)
